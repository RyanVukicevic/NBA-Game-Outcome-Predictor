...
def ingest_odds(ledger, events, received=None, bookmakers=("draftkings", "fanduel")):
    ...
    stats = dict(matched=0, unmatched=0, started=0, quotes=0, unavailable=0, invalid=0)
    matched_ids = set()
    ...
    for event in events:
        ...
        # Fail closed on schedule disagreements rather than merging distinct games.
        matches = [g for g in candidates if g["status"] == "scheduled" and g["tipoff"] == tipoff]
        if len(matches) != 1:
            stats["unmatched"] += 1
            continue
        gid = matches[0]["id"]
        ...
        for bookmaker in bookmakers:
            ...
            if valid and updated:
                try:
                    ledger.quote(gid, bookmaker, updated, tipoff, prices[event["home_team"]],
                                 prices[event["away_team"]], received, payload=payload)
                    stats["quotes"] += 1
                    continue
                except (ValueError, TypeError):
                    stats["invalid"] += 1
            ledger.quote(gid, bookmaker, received, tipoff, received=received,
